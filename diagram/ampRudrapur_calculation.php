...
$iverterDCData = array();
$device =array('11621','11618','11617','11620','11619');
...
$inverterACData1 =getdeviceData($startTime,$endTime,11621,'Pac');
$inverterDCData1 =getdeviceData1($startTime,$endTime,11621,$dcPowField);


$inverterACData2 =getdeviceData($startTime,$endTime,11618,'Pac');
$inverterDCData2 =getdeviceData1($startTime,$endTime,11618,$dcPowField);

$inverterACData3 =getdeviceData($startTime,$endTime,11617,'Pac');
$inverterDCData3 =getdeviceData1($startTime,$endTime,11617,$dcPowField);

$inverterACData4 =getdeviceData($startTime,$endTime,11620,'Pac');
$inverterDCData4 =getdeviceData1($startTime,$endTime,11620,$dcPowField);

$inverterACData5 =getdeviceData($startTime,$endTime,11619,'Pac');
$inverterDCData5 =getdeviceData1($startTime,$endTime,11619,$dcPowField);
...
foreach ($inverterDCVolt as $inverterDCVoltageData){
	...
	$stime= $dcindex1-90;
	$etime=$dcindex1+90;
	$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12690') and (field='Solar_Radiation') and value > 600");
				...
				if($acindex1 == $dcindex1)
					{
						...
						$stime= $acindex1-90;
						$etime= $acindex1+90;
						$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12690') and (field='Solar_Radiation') and value > 250");
						$numCount1 =  mysql_num_rows($qry_irrad1); 
						if($numCount1!=0)
						{
							$fetchdata1=mysql_fetch_array($qry_irrad1);
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts1 and (device='12690') and (field='Module_Temperature')");
							...
							$expPower=$irradation1 * 500 *(1.9503) * (0.154);
							$actualPower = $acPower[0];
							$pr= round((($actualPower/$expPower)*100),2);
							
							$insq1="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld."','".$pr."'),('".$acindex1."',12690,'".$inv_irr."','".$irradation1."'),('".$acindex1."',12690,'".$ACmodule_fld1."','".$module_tempVal1."')";
							echo "<br>".$insq1."<br>";
							$iqry1= mysql_query($insq1) or die(mysql_error());
						}
						
						//////above 600
						$qry_irrad2=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12690') and (field='Solar_Radiation') and value > 600");
						...
						if($numCount2!=0)
						{
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts2 and (device='12690') and (field='Module_Temperature')");
							$fetchdata3=mysql_fetch_array($qry_module);
							$module_tempVal2 =$fetchdata3['value'];
							$ACmodule_fld2="AC_Module_Temp_600";
							$pr_fld2="Inv_AC_PR";
							$inv_irr2 = "Inv_irrad_600";
							$expPower=$irradation2 * 500 * (1.9503) * (0.154);
							$actualPower = $acPower[0];
							$pr2= round((($actualPower/$expPower)*100),2);
							//print_r($acPower[0]);echo '/'.$expPower.'<br>';
							
							$insq2="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld2."','".$pr2."'),('".$acindex1."',12690,'".$inv_irr2."','".$irradation2."'),('".$acindex1."',12690,'".$ACmodule_fld2."','".$module_tempVal2."')";
							echo "<br>".$insq2."<br>";
...
//Energy meter system PR
  ...
  $qry_irrad4=mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts > $startTime and ts < $endTime and  (device='12690') and (field='Solar_Radiation') and value > 250");
   
   while($rlt = mysql_fetch_array($qry_irrad4)){
  	 $timestamp = $rlt ['ts'];
  	 $En_irradation1 = $rlt['groupval'];

		  $qry_enacpower = mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='7772') and (field='Activepower_Total')");
      ...
      $qry_temp =mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='12690') and (field='Module_Temperature')");
      $numCount2 =  mysql_num_rows($qry_temp); 
		...
		$SPR = round((($En_ActPowValue)/(($En_irradation1*500*(1.9503)*0.154)/1000))*100,2); 
        $insq4="REPLACE ".$table."(ts,device,field,value) values('".$timestamp."',7772,'".$System_PR."','".$SPR."'),('".$timestamp."',12690,'".$Energy_irrad."','".$En_irradation1."'),('".$timestamp."', 12690 ,'".$En_Module_Temp."','".$En_ModuleValue."'),('".$timestamp."', 7772 ,'".$En_ActPowFld."','".$En_ActPowValue."')";
		//echo "<br>".$insq4."<br>";

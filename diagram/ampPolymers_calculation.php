...
$iverterDCData = array();
$device =array('12660','12659','12658');
...
$inverterACData1 =getdeviceData($startTime,$endTime,12660,'PAC_Total');
$inverterDCData1 =getdeviceData($startTime,$endTime,12660,'PDC_Total');


$inverterACData2 =getdeviceData($startTime,$endTime,12659,'PAC_Total');
$inverterDCData2 =getdeviceData($startTime,$endTime,12659,'PDC_Total');

$inverterACData3 =getdeviceData($startTime,$endTime,12658,'PAC_Total');
$inverterDCData3 =getdeviceData($startTime,$endTime,12658,'PDC_Total');



//print_r($inverterDCVolt);
...
foreach ($inverterDCVolt as $inverterDCVoltageData){
	...
	$stime= $dcindex1-90;
	$etime=$dcindex1+90;
	$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12417') and (field='Solar_Radiation') and value > 600");
				...
				if($acindex1 == $dcindex1)
					{
						...
						$stime= $acindex1-110;
						$etime= $acindex1+110;
						$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12417') and (field='Solar_Radiation') and value > 250");
						$numCount1 =  mysql_num_rows($qry_irrad1); 
						if($numCount1!=0)
						{
							$fetchdata1=mysql_fetch_array($qry_irrad1);
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts1 and (device='12417') and (field='Module_Temperature')");
							...
							$expPower=($irradation1 * 220 *(1.8721) * (0.155))/1000;
							$actualPower = $acPower[0]/100;
							$pr= round((($actualPower/$expPower)*100),2);
							
							$insq1="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld."','".$pr."'),('".$acindex1."',12417,'".$inv_irr."','".$irradation1."'),('".$acindex1."',12417,'".$ACmodule_fld1."','".$module_tempVal1."')";
							//echo "<br>".$insq1."<br>";
							$iqry1= mysql_query($insq1) or die(mysql_error());
						}
						
						//////above 600
						$qry_irrad2=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12417') and (field='Solar_Radiation') and value > 600");
						...
						if($numCount2!=0)
						{
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts2 and (device='12417') and (field='Module_Temperature')");
							$fetchdata3=mysql_fetch_array($qry_module);
							$module_tempVal2 =$fetchdata3['value'];
							$ACmodule_fld2="AC_Module_Temp_600";
							$pr_fld2="Inv_AC_PR";
							$inv_irr2 = "Inv_irrad_600";
							$expPower=($irradation2 * 220 * (1.8721) * (0.155))/1000;
							$actualPower = $acPower[0]/100;
							$pr2= round((($actualPower/$expPower)*100),2);
							//print_r($acPower[0]);echo '/'.$expPower.'<br>';
							
							$insq2="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld2."','".$pr2."'),('".$acindex1."',12417,'".$inv_irr2."','".$irradation2."'),('".$acindex1."',12417,'".$ACmodule_fld2."','".$module_tempVal2."')";
							//echo "<br>".$insq2."<br>";
...
//Energy meter system PR
  ...
  $qry_irrad4=mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts > $startTime and ts < $endTime and  (device='12417') and (field='Solar_Radiation') and value > 250");
   ...
   while($rlt = mysql_fetch_array($qry_irrad4)){
	...
	$etime= $rlt ['ts']+100;
  	 $En_irradation1 = $rlt['groupval'];
		  ...
		  $qry_enacpower = mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12037') and (field='Activepower_Total')");
		  $numCounten =  mysql_num_rows($qry_enacpower); 
		  if( $numCounten!=0){ 
				$fetchdata5=mysql_fetch_array($qry_enacpower);
				echo $En_ActPowValue =$fetchdata5['groupval']; 
				
			}
      $qry_temp =mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='12417') and (field='Module_Temperature')");
      $numCount2 =  mysql_num_rows($qry_temp); 
		...
		$expPower = ($En_irradation1*660*(1.8721)*0.155)/1000;
		$SPR = round(($En_ActPowValue / $expPower)*100,2); 
        $insq4="REPLACE ".$table."(ts,device,field,value) values('".$timestamp."',12037,'".$System_PR."','".$SPR."'),('".$timestamp."',12417,'".$Energy_irrad."','".$En_irradation1."'),('".$timestamp."', 12417 ,'".$En_Module_Temp."','".$En_ModuleValue."'),('".$timestamp."', 12037 ,'".$En_ActPowFld."','".$En_ActPowValue."')";
		echo "<br>".$insq4."<br>";

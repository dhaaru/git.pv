    ...
    if ($sens_gate != 0 and $sens_node != 0) {
        ...
        if ($ein_sens != 0 && !in_array($sens_gate . "_" . $sens_node . "_" . $ein_sens, $radNames)) {
            if ($type == "wetter" || sizeof($radNames) == 0) {
                $eins++;
                if ($sens_gate == 2001) {
                    $radNames [] = "Block A1 Pyranometer";
                } else if ($sens_gate == 2061) {
                    $radNames [] = "Control Room Pyranometer";
                } else {
...
if (false && $park_no == "25" && $phase == "tag" && $type == "s0") {
    header("location: argdiagram5.php?args=args=;0,1,0,108,SR1_15,Irradiation,15,W/mSQUA,0;0,1,0,108,AT_15,Temperature,15,DEGC,0;0,1,0,109,PAC,Inverter 1,15,kW,2;0,1,0,110,PAC,Inverter 2,15,kW,4;0,1,0,111,PAC,Inverter 3,15,kW,5;0,1,0,112,PAC,Inverter 4,15,kW,6&defaults=0,1,2,3,4,5&hideClear=1&hideDelta=1&stamp=" . $tsMin . "&endstamp=" . $tsMax);
}

if (false && $park_no == "25" && $phase == "tag" && $type == "wr") {
    header("location: argdiagram5.php?args=0,1,0,109,PAC,Inverter 1,15,kW,2;0,1,0,110,PAC,Inverter 2,15,kW,4;0,1,0,111,PAC,Inverter 3,15,kW,5;0,1,0,112,PAC,Inverter 4,15,kW,6&defaults=0,1,2,3,4,5&hideDelta=1&hideClear=1&stamp=" . $tsMin . "&endstamp=" . $tsMax);
}

if (false && $park_no == "25" && $phase == "tag" && $type == "wetter") {
    header("location: argdiagram5.php?args=;0,1,0,108,SR1,Irradiation,15,W/mSQUA,0;0,1,0,108,SR2,Irradiation inclined,15,W/mSQUA,3;0,1,0,108,AT,Temperature,15,DEGC,0&defaults=0,1,2&hideDelta=1&hideClear=1&stamp=" . $tsMin . "&endstamp=" . $tsMax);
//    $diagrammCode .= <tr><td><iframe id="frame1" width="99%" height="99%" SRC="diagram/argdiagram5.php?args=;0,1,0,108,SR1_15,Irradiation,15,W/mSQUA,0;0,1,0,108,SR2_15,Irradiation inclined,15,W/mSQUA,3;0,1,0,108,AT_15,Temperature,15,DEGC,0&defaults=0,1,2&hideClear=1&hideDelta=1&stamp=1358879400&endstamp=1359142200" border="0"></iframe></td></tr><br>
}
        ...
        <div style="height: 94%; width: 99%; text-align: center;">
            ...
            if ($totalProduction > 0) {
                ...
                if ($park_no == "20") {
                    ...
                    $query = "select device, max(value) as value from _devicedatavalue where ts < $tsMax and ts > " . ($tsMin) . " and field ='e_total' and device in (182, 181) group by device";
                    $ds1 = mysql_query($query, $verbindung) or die(mysql_error());
                    ...
                    echo '<p style = "font-size: 90%">Total Production<br>' . number_format($sum, 2, ".", "") . ' kWh</p>';
                }
            }
// && $park_no != "20"
                     ...
                     id="buttons">
                         ...
                         if ($phase == "tag") {
                             if ($park_no == 20) {
                                 ...
                                 echo '<a href="export2.php?jahr=' . $jahr . '&mon=' . $mon . '&tag=' . $tag . '&phase=' . $phase . '&stamp=' . $tsMin . '&endstamp=' . ($tsMin + 24 * 3600) . '&args=0,13,0,5,U1_900,A1%20Irradiation%201,15,W/mSQUA,17;-22.68,1.15,0,5,U2_900,A1%20Module%20Temperature,15,DEGC,1;0,1,0,5,T3_900,A1%20Sensor%20Temperature,15,DEGC,12;-19.0476,21,0,5,U4_900,A1%20Pyranometer,15,W/mSQUA,6;0,13,0,50,U1_900,A7%20Irradiation,15,W/mSQUA,9;-22.68,1.15,0,50,U2_900,A7%20Sensor%20Temperature,15,DEGC,4;0,1,0,50,T3_900,A7%20Module%20Temperature,15,DEGC,13;-19.0476,21,0,180,U1_900,WS%20Radiation,15,W/mSQUA,4;-30,1,0,180,U2_900,WS%20Ambient%20Temperature,15,DEGC,8;0,1,0,180,T3_900,WS%20Module%20Temperature,15,DEGC,19;0,0.5,0,178,U2_900,WS%20Wind%20speed,15,m/s,1;0,3.6,0,178,U1,WS%20Wind%20direction,15,degree,0;0,1,4068000,182,E_Total,Plant%20Trafo%201%20Energy%20Total,15,kWh,18;0,1,4068000,181,E_Total,Plant%20Trafo%202%20Energy%20Total,15,kWh,1&showQueries=0&delta=0">';
                             } else {
...
if (sizeof($tmpValues [0]) > 0) {
    ...
    foreach ($tmpValues as $temp) {
        ...
        $lastTs = $tsMin;
        foreach ($temp as $val) {
            if (false && $phase == "tag" && ($val[0] - $lastTs) > 1201) {
...
if (sizeof($einValues [0]) > 0) {
    ...
    foreach ($einValues as $ein) {
        ...
        $lastTs = $tsMin;
        foreach ($ein as $val) {
            if (false && $phase == "tag" && ($val[0] - $lastTs) > 1200) {

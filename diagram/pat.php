...
if ($portal == "20") {
    ...
    while ($row_ds1 = mysql_fetch_array($ds1)) {
        ...
        $a1['' . floor($row_ds1['ts'] / 300)] = "=" . $row_ds1['temp'] . "\t=" . $row_ds1['irr1'] . "\t=" . $row_ds1['irr2'] . "\t=" . $row_ds1['ws'] . "\t=" . $row_ds1['wd'];
        $tstamps[] = '' . floor($row_ds1['ts'] / 300);
    }
    ...
    while ($row_ds1 = mysql_fetch_array($ds1)) {
        ...
        if (!in_array('' . floor($row_ds1['ts'] / 300), $tstamps)) {
            $tstamps[] = '' . floor($row_ds1['ts'] / 300);
        }
        $energy['' . floor($row_ds1['ts'] / 300)] = "\t=" . $row_ds1['e_total'];
    }
    ...
    while ($row_ds1 = mysql_fetch_array($ds1)) {
        ...
        $ds2 = mysql_query($query, $verbindung) or die(mysql_error());
        while ($row_ds2 = mysql_fetch_array($ds2)) {
            if (!in_array('' . floor($row_ds2['ts'] / 300), $tstamps)) {
                $tstamps[] = '' . floor($row_ds2['ts'] / 300);
            }
            $wrs[$wrIndex][1]['' . floor($row_ds2['ts'] / 300)] = "\t=" . $row_ds2[value];
        }
        ...
        $query = "select DISTINCT ts, e_total, u_ac_ph1, u_ac_ph2, u_ac_ph3, i_ac_ph1, i_ac_ph2, i_ac_ph3 from 2531_messdaten_wr where sn = '" . $row_ds1['sn'] . "' and ts < $endstamp and ts > " . $stamp . " order by ts";
        $ds2 = mysql_query($query, $verbindung) or die(mysql_error());
        while ($row_ds2 = mysql_fetch_array($ds2)) {
            if (!in_array('' . floor($row_ds2['ts'] / 300), $tstamps)) {
                $tstamps[] = '' . floor($row_ds2['ts'] / 300);
            }
            $wrs[$wrIndex][1]['' . floor($row_ds2['ts'] / 300)] = "" . $row_ds2[e_total] . $wrs[$wrIndex][1]['' . floor($row_ds2['ts'] / 300)] . "\t=" . $row_ds2[u_ac_ph1] . "\t=" . $row_ds2[u_ac_ph2] . "\t=" . $row_ds2[u_ac_ph3] . "\t=" . $row_ds2[i_ac_ph1] . "\t=" . $row_ds2[i_ac_ph2] . "\t=" . $row_ds2[i_ac_ph3];
        }

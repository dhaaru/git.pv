...
$argString = "";
if ($echoArgs == 1) {
    $argString = '<tr><td><iframe id="frame1" width="99%" height="99%" SRC="diagram/argdiagram5.php?args=' . $args . '&defaults=' . $defaults.'&hideClear=1&hideDelta=1&stamp=' . $stamp . '&endstamp=' . $endstamp . '" border="0"></iframe></td></tr><br>'; $count++;
...
foreach ($args as $arg) {
    $words = split(',', $arg);
    if (sizeof($words) != 10 && sizeof($words) != 9) {
        continue;
    }if (sizeof($words==9)){
        $words[]=0;
    ...
    $devices[$words[3]][bartype]=$words[10];
...
foreach ($values as $devkey => $device) {
    ...
    foreach ($device[data] as $tskey => $value) {
        ...
        echo "devValues['" . $index . "'].push([" . ($tskey * 1000 + ($index*24*3600000/$devices)) . ", " . $value[value] . "]);\n";
    }
    $index++;
}
?>
    ...
    function plotWithOptions(){
        ...
        var options = 
            ...
            ,bars: {show: true, barWidth: (3600000*24/<?php echo $devices;?>), align: 'left'}
            ,points: {show: true}
            ,yaxes: [ 
<?php
$first = true;
...
foreach ($values as $key => $value) {
    ...
    echo "{ lines: {show: false}, points: {show: false}, yaxis: " . ($value[axis] + 1) . ", color: $value[color], label: '" . $key . $axisWord . ", data: devValues['" . ($index++) . "']}\n";
}

...
$sourcetable = "fortis_calculation";
if ($compressed) {
...
{
	$query="select ts,value from fortis_calculation where ts > $startTime and ts < $endTime and (device='".$deviceId."') and (field='".$field."')";
...
if(($phase == 'FortINV1ACPR' && $park_no == 72) || ($phase == 'FortINV2ACPR' && $park_no == 72))
	{ 
		$Module_temp_600=getdeviceData($startTime,$endTime,13475,'AC_Module_Temp_600');

		foreach($Module_temp_600 as $moduleindex1=>$modulevalue){
		$ts= $moduleindex1;
		$mod_value = $modulevalue;
		$qry_inverter1 = mysql_query("select  ROUND((value),3) as groupval,ts FROM fortis_calculation where ts=$ts and (device='13478') and (field='Inv_AC_PR')");
		...
		$qry_inverter2 = mysql_query("select  ROUND((value),3) as groupval,ts FROM fortis_calculation where ts=$ts and (device='13479') and (field='Inv_AC_PR')");
	...
	if(($phase == 'FortINV1DCPR' && $park_no == 72) || ($phase == 'FortINV2DCPR' && $park_no == 72))
	{
		...
		$ts= $moduleindex1;
		$mod_value = $modulevalue;
		$qry_inverterdc1 = mysql_query("select  ROUND((value),3) as groupval,ts FROM fortis_calculation where ts=$ts and (device='13478') and (field='DC_Vol_Coeff')");
		...
		$qry_inverterdc2 = mysql_query("select  ROUND((value),3) as groupval,ts FROM fortis_calculation where ts=$ts and (device='13479') and (field='DC_Vol_Coeff')");
	...
	if(($phase == 'FortINV1EFF' && $park_no == 72) || ($phase == 'FortINV2EFF' && $park_no == 72))
	{
		$inver1Eff =getdeviceData($startTime,$endTime,13478,'Inv1_Eff');
		$inver2Eff =getdeviceData($startTime,$endTime,13479,'Inv2_Eff');
		
		 //print_r($inver1Eff);exit;
		if($inver1Eff!=''){
			foreach($inver1Eff as $inv1index1=>$inv1eff1){
			$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval FROM _devicedatavalue where ts=$inv1index1 and (device='13475') and (field='Irradiance')");
	...
	if($inver2Eff!=''){
		foreach($inver2Eff as $inv2index2=>$inv2eff2){
					$qry_irrad2=mysql_query("select  ROUND((value),3) as groupval FROM _devicedatavalue where ts=$inv2index2 and (device='13475') and (field='Irradiance')");
                   ...
                   <!-- <input title="Reset the zoom"    style="flow: left;" id="resetZoom" onClick="resetZoom()" type="image"    src="../imgs/lupe_grey.png" disabled>-->
                    ...
                    if ($exception != 25) {
                        if ($anyArgs) {
							...
							if($phase == "FortINV1EFF" && $park_no=="72"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,13478,Inv1_Eff,Inverter1,15,%,8;0,1,0,13475,Irradiance,Irradiation,5,W/m&sup2;,\'Gold\';">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
							
							if($phase == "FortINV2EFF" && $park_no=="72"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,13479,Inv2_Eff,Inverter2,15,%,8;0,1,0,13475,Irradiance,Irradiation,5,W/m&sup2;,\'Gold\';">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
												
							
							 if($phase == "FortINV1ACPR" && $park_no=="72"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,13478,Inv_AC_PR,Inv1%20AC%20PR,15,%,4;0,1,0,13475,Inv_irrad_600,Irradiation,15,W/m&sup2;,\'Gold\';0,1,0,13475,AC_Module_Temp_600,Module Temperature,15,&deg;C,\'darkred\'">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}  
							if($phase == "FortINV2ACPR" && $park_no=="72"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,13479,Inv_AC_PR,Inv2%20AC%20PR,15,%,4;0,1,0,13475,Irradiance,Irradiation,15,W/m&sup2;,\'Gold\';0,1,0,13475,AC_Module_Temp_600,Module Temperature,15,&deg;C,\'darkred\'">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							} 
						
							
							if($phase == "FortINV1DCPR" && $park_no=="72"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,13478,DC_Vol_Coeff,Inv 1(DC_Voltage),15,V,4;0,1,0,13475,Inv_irrad_600,Irradiation,15,W/m&sup2;,\'Gold\';0,1,0,13475,AC_Module_Temp_600,Module Temperature,15,&deg;C,\'darkred\'">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
							if($phase == "FortINV2DCPR" && $park_no=="72"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,13479,DC_Vol_Coeff,Inv 2(DC_Voltage),15,V,4;0,1,0,13475,Inv_irrad_600,Irradiation,15,W/m&sup2;,\'Gold\';0,1,0,13475,AC_Module_Temp_600,Module Temperature,15,&deg;C,\'darkred\'">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
...
<?php if(($phase == 'FortINV1EFF' && $park_no == 72) || ($phase == 'FortINV2EFF' && $park_no == 72)){?>
	$(function() {

			var datasets = {
			"Inv Efficency": {
				
				<?php if($phase == 'FortINV1EFF'){?>
				label: "Inv1 Efficency", points: { symbol: "circle" },
				data: <?php echo $inverEff1; ?>
				<?php } else if($phase == 'FortINV2EFF'){?>
	...
	<?php }elseif(($phase == 'FortINV1ACPR' && $park_no == 72) || ($phase == 'FortINV2ACPR' && $park_no == 72)){?>
	$(function() {

		var data = [
			<?php if($phase == 'FortINV1ACPR'){?>
			{ data: <?php echo $invACPR1; ?>, points: { symbol: "diamond" }, label: "INV1 AC PR (%)" }
			<?php } else if($phase == 'FortINV2ACPR'){?>
	...
	<?php }elseif(($phase == 'FortINV1DCPR' && $park_no == 72) || ($phase == 'FortINV2DCPR' && $park_no == 72)){?>
	$(function() {

		var data = [
			<?php if($phase == 'FortINV1DCPR'){?>
			{ data: <?php echo $inverDCPR1; ?>, points: { symbol: "circle" }, label: "INV1 DC Voltage(V)" }
			<?php } else if($phase == 'FortINV2DCPR'){?>

...
$iverterDCData = array();
$device =array('12028','12024','12021','12023','12027','12026');
...
$inverterACData1 =getdeviceData($startTime,$endTime,12028,'Pac');
$inverterDCData1 =getdeviceData1($startTime,$endTime,12028,$dcPowField);


$inverterACData2 =getdeviceData($startTime,$endTime,12024,'Pac');
$inverterDCData2 =getdeviceData1($startTime,$endTime,12024,$dcPowField);

$inverterACData3 =getdeviceData($startTime,$endTime,12021,'Pac');
$inverterDCData3 =getdeviceData1($startTime,$endTime,12021,$dcPowField);

$inverterACData4 =getdeviceData($startTime,$endTime,12023,'Pac');
$inverterDCData4 =getdeviceData1($startTime,$endTime,12023,$dcPowField);

$inverterACData5 =getdeviceData($startTime,$endTime,12027,'Pac');
$inverterDCData5 =getdeviceData1($startTime,$endTime,12027,$dcPowField);

$inverterACData6 =getdeviceData($startTime,$endTime,12026,'Pac');
$inverterDCData6 =getdeviceData1($startTime,$endTime,12026,$dcPowField);
...
foreach ($inverterDCVolt as $inverterDCVoltageData){
	...
	$stime= $dcindex1-90;
	$etime=$dcindex1+90;
	$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12029') and (field='Solar_Radiation') and value > 600");
			...
			if($numCount1!=0)
				{	 
					...
					if($ts1>=$stime || $ts1<=$etime){
						...
						$DC_Vol_Coeff_Val= round((($dcVoltage[0]/(21*45.4))*100),2);
				...
				if($acindex1 == $dcindex1)
					{
						...
						$stime= $acindex1-90;
						$etime= $acindex1+90;
						$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12029') and (field='Solar_Radiation') and value > 250");
						$numCount1 =  mysql_num_rows($qry_irrad1); 
						if($numCount1!=0)
						{
							$fetchdata1=mysql_fetch_array($qry_irrad1);
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts1 and (device='12029') and (field='Module_Temperature')");
							...
							$expPower=$irradation1 * 84 *(1.9503) * (0.147);
							$actualPower = $acPower[0];
							$pr= round((($actualPower/$expPower)*100),2);
							
							$insq1="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld."','".$pr."'),('".$acindex1."',12029,'".$inv_irr."','".$irradation1."'),('".$acindex1."',12029,'".$ACmodule_fld1."','".$module_tempVal1."')";
							echo "<br>".$insq1."<br>";
							$iqry1= mysql_query($insq1) or die(mysql_error());
						}
						
						//////above 600
						$qry_irrad2=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='12029') and (field='Solar_Radiation') and value > 600");
						...
						if($numCount2!=0)
						{
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts2 and (device='12029') and (field='Module_Temperature')");
							$fetchdata3=mysql_fetch_array($qry_module);
							$module_tempVal2 =$fetchdata3['value'];
							$ACmodule_fld2="AC_Module_Temp_600";
							$pr_fld2="Inv_AC_PR";
							$inv_irr2 = "Inv_irrad_600";
							$expPower=$irradation2 * 84 * (1.9503) * (0.147);
							$actualPower = $acPower[0];
							$pr2= round((($actualPower/$expPower)*100),2);
							//print_r($acPower[0]);echo '/'.$expPower.'<br>';
							
							$insq2="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld2."','".$pr2."'),('".$acindex1."',12029,'".$inv_irr2."','".$irradation2."'),('".$acindex1."',12029,'".$ACmodule_fld2."','".$module_tempVal2."')";
							echo "<br>".$insq2."<br>";
...
//Energy meter system PR
  ...
  $qry_irrad4=mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts > $startTime and ts < $endTime and  (device='12029') and (field='Solar_Radiation') and value > 250");
   
   while($rlt = mysql_fetch_array($qry_irrad4)){
  	 $timestamp = $rlt ['ts'];
  	 $En_irradation1 = $rlt['groupval'];

		  $qry_enacpower = mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='11615') and (field='Activepower_Total')");
      ...
      $qry_temp =mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='12029') and (field='Module_Temperature')");
      $numCount2 =  mysql_num_rows($qry_temp); 
		...
		$expPower = ($En_irradation1*504*(1.9503)*0.147)/1000;
		$SPR = round(($En_ActPowValue / $expPower)*100,2); 
        $insq4="REPLACE ".$table."(ts,device,field,value) values('".$timestamp."',11615,'".$System_PR."','".$SPR."'),('".$timestamp."',12029,'".$Energy_irrad."','".$En_irradation1."'),('".$timestamp."', 12029 ,'".$En_Module_Temp."','".$En_ModuleValue."'),('".$timestamp."', 11615 ,'".$En_ActPowFld."','".$En_ActPowValue."')";
		//echo "<br>".$insq4."<br>";

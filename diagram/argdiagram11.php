...
# ------------------------------------------------
print ("<p><sub>argdiagram11 influx clean version  'amplus_all_calculations' </sub></p>");
#################################################
...
if(($phase == 'INVACPR' && $park_no == 46) )
  {
    $Module_temp_600=getdeviceData($startTime,$endTime,9971,'AC_Module_Temp_600');

    foreach($Module_temp_600 as $moduleindex1=>$modulevalue){
    $ts= $moduleindex1;
    $mod_value = $modulevalue;
     $qry_inverter1 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=46 AND ts=$ts and (device='9971') and (field='Inv_AC_PR_600')");
    $numCount1 =  mysql_num_rows($qry_inverter1); 

    if($numCount1!=0)
    {
      $fetchdata1=mysql_fetch_array($qry_inverter1);
      //print_r($qry_inverter1);
      $inv1ACpr = $fetchdata1['groupval'];
      $result1[] = '['.$mod_value.','.$inv1ACpr.']';
    }
    $qry_inverter2 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=46 AND ts=$ts and (device='9972') and (field='Inv_AC_PR_600')");
    ...
    $qry_inverter3 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=46 AND ts=$ts and (device='9973') and (field='Inv_AC_PR_600')");
  ...
  if(($phase == 'INVDCPR' && $park_no == 46))
  {
    ...
    $ts= $moduleindex1;
    $mod_value = $modulevalue;
    $qry_inverterdc1 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=46 AND ts=$ts and (device='9971') and (field='Inv_DC_Vol_Coeff')");
    $dcnumCount1 =  mysql_num_rows($qry_inverterdc1); 
    if($dcnumCount1!=0)
    {
      $dcfetchdata1=mysql_fetch_array($qry_inverterdc1);
      echo $inv1DCpr = $dcfetchdata1['groupval'];
      $dcresult1[] = '['.$mod_value.','.$inv1DCpr.']';
    }
    $qry_inverterdc2 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=46 AND ts=$ts and (device='9972') and (field='Inv_DC_Vol_Coeff')");
    ...
    $qry_inverterdc3 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=46 AND ts=$ts and (device='9973') and (field='Inv_DC_Vol_Coeff')");
...
if(($phase == 'INVEFF' && $park_no == 46))
  {
    $inver1Eff =getdeviceData($startTime,$endTime,9971,'Inv_Eff');
    $inver2Eff =getdeviceData($startTime,$endTime,9972,'Inv_Eff');
    $inver3Eff =getdeviceData($startTime,$endTime,9973,'Inv_Eff');
    ...
    $inverEff1 = combineIrradWithInverterEff($inver1Eff,9957,'Solar_Radiation');
  }
  if ($inver2Eff) {
    $inver2Eff = alignTimestamps($inver2Eff);
    $inverEff2 = combineIrradWithInverterEff($inver2Eff,9957,'Solar_Radiation');

  }
  if ($inver3Eff) {
    $inver3Eff = alignTimestamps($inver3Eff);
    $inverEff3 = combineIrradWithInverterEff($inver3Eff,9957,'Solar_Radiation'); 
  }
...
<!-- Excel sheet link start -->
        <div style="position:absolute; right:10px; top:10px;" id="buttons">
          ...
          if($row_export==1){
              ...
              if($phase == "INVEFF" && $park_no=="46"){
                echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,9971,Inv1_Eff,Inverter1,15,%,8;0,1,0,9972,Inv2_Eff,Inverter2,15,%,9;0,1,0,9973,Inv3_Eff,Inverter3,15,%,9;0,1,0,9957,Solar_Radiation,Irradiation,5,W/m&sup2;,\'Gold\';">';
               ...
               if($phase == "INVACPR" && $park_no=="46"){
                echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,9971,Inv_AC_PR,Inverter1%20PR,15,%,4;0,1,0,9972,Inv_AC_PR,Inverter2%20PR,15,%,5;0,1,0,9973,Inv_AC_PR,Inverter3%20PR,15,%,4;0,1,0,9957,Inv_irrad_600,Irradiation,15,W/m&sup2;,\'Gold\';0,1,0,9957,AC_Module_Temp_600,Module Temperature,15,&deg;C,\'darkred\'">';
                echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
                echo '</a>';
              }  
              if($phase == "INVDCPR" && $park_no=="46"){
                echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,9971,DC_Vol_Coeff,Inv 1(DC_Voltage),15,V,4;0,1,0,9972,DC_Vol_Coeff,Inv 2(DC_Voltage),15,V,5;0,1,0,9973,DC_Vol_Coeff,Inv 3(DC_Voltage),15,V,6;0,1,0,9957,Inv_irrad_600,Irradiation,15,W/m&sup2;,\'Gold\';0,1,0,9957,AC_Module_Temp_600,Module Temperature,15,&deg;C,\'darkred\'">';
                echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
                echo '</a>';
              }
                        }
...
<?php if(($phase == 'INVEFF' && $park_no == 46)){ ?>
  $(function() {
  ...
  <?php }elseif(($phase == 'INVACPR' && $park_no == 46)){?>
  $(function() {
  ...
  <?php }elseif(($phase == 'INVDCPR' && $park_no == 46)){?>
  $(function() {

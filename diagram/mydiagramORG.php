	...
	function fillData(){
		...
		if (selectedItem=="Yield" || selectedItem == "Inst. yield"){
			...
			for($i = 0; $i < $indexes [$specYield]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$specYield] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$specCurrent]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$specCurrent] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$specPower]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$specPower] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>

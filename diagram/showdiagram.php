    ...
    while ($row_ds1 = mysql_fetch_array($ds1)) {
        ...
        if ($row_ds1[source] == null) {
            ...
            if ($device[sumtype] == 1) {
                ...
                if ($showCalc) {
                    echo "device.sumtype=1 " . $tmp . "<br>";
                }

                $value += $tmp;
            } else if ($device[sumtype] == 0) {
                $tmp = round(subcalc($ndevice, $verbindung, $data, $showCalc), 3);
                if ($showCalc) {
                    echo "device.sumtype=0 " . $tmp . "<br>";
                }
                $value *= $tmp;
            }
        } else {
            if ($device[sumtype] == 1) {
                if ($data[$row_ds1[source]][OK]) {
                    $tmp = round((($data[$row_ds1[source]][data][$row_ds1[sn]][$row_ds1[field]][value] + $row_ds1[elementpreoffset]) * $row_ds1[elementfactor] + $row_ds1[elementpostoffset]), 3);
                    if ($showCalc) {

                        echo "device.sumtype=1 " . $tmp . "<br>";
                        echo "source: $row_ds1[source] sn: $row_ds1[sn] field: $row_ds1[field] <br>";
                    }
                    $value += $tmp;
                } else {
                    $value = 0;
...
$alarmtexts = array();
# L&T Rooftop (Kaco, SoilingLoss) - No connection
if ($diagram == 11) {
//    int mktime ([ int $hour = date("H") [, int $minute = date("i") [, int $second = date("s") [, int $month = date("n") [, int $day = date("j") [, int $year = date("Y") [, int $is_dst = -1 ]]]]]]] )
    ...
    $query = "SELECT value, ts from _devicedatavalue 
      WHERE device=477 and field = 'E_Total' 
      AND ts < " . $yesterdate . " and ts > " . ($yesterdate - 3600 * 24) . " 
      ORDER BY ts DESC";
    if ($showQueries == 1) {
        echo $query . $ln;
    }
    ...
    $query = "SELECT value, ts 
      FROM _devicedatavalue 
      WHERE device=477 and field = 'E_Total' 
      AND ts < " . $monthdate . " and ts > " . ($monthdate - 3600 * 24) . " 
      ORDER BY ts DESC";
    if ($showQueries == 1) {
        echo $query . $ln;
    }
    ...
    $query = "SELECT value, ts 
      FROM _devicedatavalue 
      WHERE device=477 and field = 'E_Total' 
      AND ts < " . $yeardate . " and ts > " . ($yeardate - 3600 * 24) . " 
      ORDER BY ts DESC";
    if ($showQueries == 1) {
        echo $query . $ln;
    }
...
$returnitems = array();
$returnitemkeys = array();

$query = "select si.* from _scadapicreturnitem as si where si.scada= $diagram";
if ($showQueries == 1) {
    echo $query . $ln;
}
...
$ds1 = mysql_query($query, $verbindung) or die(mysql_error());
while ($row_ds1 = mysql_fetch_array($ds1)) {
    $returnitems[$row_ds1[item]][x] = $row_ds1[x];
    $returnitems[$row_ds1[item]][y] = $row_ds1[y];
    $returnitems[$row_ds1[item]][source] = $row_ds1[source];
    $returnitems[$row_ds1[item]][font] = $row_ds1[font];
    $returnitems[$row_ds1[item]][field] = $row_ds1[field];
    $returnitems[$row_ds1[item]][ts] = 0;
    $returnitems[$row_ds1[item]][value] = 0;
    
    
    
    $returnitemkeys[]=$row_ds1[item];
}

foreach ($returnitemkeys as $itemkey) {
    
    $query = "select * from _scadacsvvalue where source= '".$returnitems[$itemkey][source]."' and field = '".$returnitems[$itemkey][field]."' order by ts desc limit 1";
    if ($showQueries == 1) {
        echo $query . $ln;
    }
    ...
    $ds1 = mysql_query($query, $verbindung) or die(mysql_error());
    while ($row_ds1 = mysql_fetch_array($ds1)) {
        $returnitems[$itemkey][ts]=$row_ds1[ts];
        $returnitems[$itemkey][value]=$row_ds1[value];
...
foreach ($urls as $url) {
    ...
    for ($index = 1; $index < sizeof($lines); $index++) {
        ...
        if (!is_null($dev) && !is_null($field)) {
            $query = "REPLACE _devicedatavalue 
              (ts, device, field, value) 
              VALUES 
              ($newTs, $dev, '$field', $newValue)";
            if ($showQueries == 1) {
                echo $query . $ln;
            }
...
# Kiran Charanka 20MWp
if ($diagram == 7) {

    $time = mktime(18, 30, 0, date("n"), date("j") - 1, date("Y"));
    $sum = 0;

    $query = "SELECT device, max(value) as value 
      FROM _devicedatavalue 
      WHERE ts < $time and ts > " . ($time - 86400) . " 
      AND field ='e_total' and device in (182, 181) 
      GROUP BY device";
    if ($showQueries == 1) {
        echo $query . "\r\n";
    }
    ...
    $query = "SELECT device, field, value 
      FROM _devicedatavalue 
      WHERE ts > $time 
      AND field in ('PAC', 'PDC1', 'PDC2', 'PDC3') 
      AND device in (70,71,72,73,74,75,76,77,78,79,80,81,82,83,84,85,86,87,88,89,90,91,92,93,94,95,96,97,98,99,100,101,102,103,104,105,106,107,142,143,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173) 
      ORDER BY ts DESC";
    if ($showQueries == 1) {
        echo $query . "\r\n";
    }
    ...
    $e_total = $e_total-$sum;
    $e_total = number_format(($e_total/1000), 1, ".", "");
	if	($e_total<0)
	{
    echo 'ctx.fillText(\'' . "No connection", '\',' . 433 . ',' . 164 . ');';
	}
	else
	{
		echo 'ctx.fillText(\'' . $e_total, '\',' . 433 . ',' . 164 . ');';
	}
    }
?>

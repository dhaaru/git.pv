...
foreach ($args as $arg) {
	...
	{
		$ids=implode(",",$arrvl);
   $query = "select ts+$offset+400 as ts, sum((((value+$words[0])*$words[1])+$words[2])) as value from _devicedatavalue where value is not null and device IN (".$ids.") and field = '$translatedField' and ts > $stamp and ts < $endstamp group by ts";
	}
	else
	{
		if($translatedField == 'Pac'){
			$query = "select ts+$offset+400 as ts, ROUND((((value+$words[0])*$words[1])+$words[2])/1000,2) as value from _devicedatavalue where value is not null and device = $words[3] and field = '$translatedField' and ts > $stamp and ts < $endstamp";
		}else{
			$query = "select ts+$offset+400 as ts, (((value+$words[0])*$words[1])+$words[2]) as value from _devicedatavalue where value is not null and device = $words[3] and field = '$translatedField' and ts > $stamp and ts < $endstamp";
		}
	}
    $header.="\t" . $words[5];

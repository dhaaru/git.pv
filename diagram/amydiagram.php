...
if ($phase == "mon") {
	...
	$tsMax = mktime ( 1, 0, 0, $mon, $tag+1, $jahr );
}
...
if ($phase=="tag"){
	$bezeichnungen = array ("Current yield", "Specific Yield", "Current", "Specific Current", "Voltage", "Power", "Specific Power", "Link voltage" );
}else {
...
<body>
	...
	<div style="height: 94%; width: 99%; text-align: center;">
		<div style="float: left; height: 95%; width: 84%; padding-top: 7px; overflow: auto;">
		...
		<div style="float: left; height: 96%; width: 15%; overflow: auto">
			...
			<div
				...
				if ($einAvg != null) {
					echo '<p style = "font-size: 80%">Average Irradiation<br>' . $einAvg . '</p>';
				}
				if ($tmpAvg != null) {
					echo '<p style = "color: red;font-size: 80%">Average Temperature<br>' . $tmpAvg . '</p>';
				}
				if ($totalProduction > 0) {
					$totalProduction = number_format ( $totalProduction, 2, ".", "" );
					echo '<p style = "font-size: 80%">Total Production<br>' . $totalProduction . ' kWh</p>';
				
				}
				
				if ($pr_anzeige != null) {
					echo '<p style = "font-size: 80%">Performance Ratio<br>' . number_format ( $pr_anzeige, 1, ".", "" ) . ' %</p>';
			...
			<div style="height: 20%; background-color: beige; overflow: auto;" id="buttons">
				...
				<br>
				 ...
				 <input title="Reset the zoom"
					style="flow: left;" id="resetZoom" onclick="resetZoom()"
					type="image" src="../imgs/lupe_grey.png" disabled> 
				<input
					title="Show/Hide temperature and irradiation data"
					style="flow: left;" id="toggleTemp" onclick="toggleTemp()"
					type="image" src="../imgs/sensor_grey.png" disabled>
			</div>
		</div>
...
?>
	function fillData(){
		...
		var s = document.getElementById('select');
		var selectedItem = s.options[s.selectedIndex].text;
		if (selectedItem=="Yield" || selectedItem == "Current yield"){
			...
			for($i = 0; $i < $indexes [$yield]; $i ++) {
				...
				echo "data.push({data: " . $variablen [$yield] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$specYield]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$specYield] . $i . ", fill: false, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$current]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$current] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$specCurrent]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$specCurrent] . $i . ", fill: false, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$voltage]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$voltage] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$power]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$power] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$specPower]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$specPower] . $i . ", fill: false, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
			...
			for($i = 0; $i < $indexes [$midVoltage]; $i ++) {
				...
				echo "var tmp = stack;";
				echo "if(zeigeKurve['" . $dataSets [$i] [0] . "']) { showLine = true; }else {tmp = null;}";
				echo "data.push({data: " . $variablen [$midVoltage] . $i . ", stack: tmp, color: ".$colors[($i%(sizeof($colors)))].", lines: {show: (showLine)}, label: '" . $dataSets [$i] [0] . "= 0" . $masz . "'});\n";
			}
			?>
		}
		...
		for (var i = 0; i< ein.length; i++){
			...
			if (!zeigeKurve[einNames[i]+'Irradiation']){
				showLine = false; }
			data.push({data: ein[i], label: einNames[i]+'Irradiation= 0W/m²', yaxis: 2, color: 20, stack: null,  lines: {align: 'center', show: (showLine), steps: false, fill: false}});
		}
		...
		for (var i = 0; i< temp.length; i++){
			...
			if (!zeigeKurve[tmpNames[i]+'Temperature']){
				showLine = false; }
			data.push({data: temp[i], label: tmpNames[i]+'Temperature= 0°C', yaxis: 3, color: 12, stack: null, lines: {align: 'center', show: (showLine), steps: false, fill: false}});
		}

...
# ------------------------------------------------
print ("<p><sub>argdiagramLap influx clean version  'amplus_all_calculations' </sub></p>");
#################################################
...
if(($phase == 'LAPINV1ACPR' && $park_no == 55) || ($phase == 'LAPINV2ACPR' && $park_no == 55) || ($phase == 'LAPINV3ACPR' && $park_no == 55) || ($phase == 'LAPINV4ACPR' && $park_no == 55))
	{ 
		$Module_temp_600=getdeviceData($startTime,$endTime,12675,'AC_Module_Temp_600');

		foreach($Module_temp_600 as $moduleindex1=>$modulevalue){
		$ts= $moduleindex1;
		$mod_value = $modulevalue;
		$qry_inverter1 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12669') and (field='Inv_AC_PR_600')");
		...
		$qry_inverter2 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12672') and (field='Inv_AC_PR_600')");
		...
		$qry_inverter3 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12670') and (field='Inv_AC_PR_600')");
		...
		$qry_inverter4 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12675') and (field='Inv_AC_PR_600')");
	...
	if(($phase == 'LAPINV1DCPR' && $park_no == 55) || ($phase == 'LAPINV2DCPR' && $park_no == 55) || ($phase == 'LAPINV3DCPR' && $park_no == 55) || ($phase == 'LAPINV4DCPR' && $park_no == 55))
	{
		...
		$ts= $moduleindex1;
		$mod_value = $modulevalue;
		$qry_inverterdc1 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12669') and (field='Inv_DC_Vol_Coeff')");
		...
		$qry_inverterdc2 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12672') and (field='Inv_DC_Vol_Coeff')");
		...
		$qry_inverterdc3 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12670') and (field='Inv_DC_Vol_Coeff')");
		...
		$qry_inverterdc4 = mysql_query("SELECT  ROUND((value),3) as groupval,ts FROM amplus_all_calculations WHERE park_no=55 AND ts=$ts and (device='12675') and (field='Inv_DC_Vol_Coeff')");
...
if(($phase == 'LAPINV1EFF' && $park_no == 55) || ($phase == 'LAPINV2EFF' && $park_no == 55) || ($phase == 'LAPINV3EFF' && $park_no == 55) || ($phase == 'LAPINV4EFF' && $park_no == 55))
	{
		$inver1Eff =getdeviceData($startTime,$endTime,12669,'Inv_Eff');
		$inver2Eff =getdeviceData($startTime,$endTime,12672,'Inv_Eff');
		$inver3Eff =getdeviceData($startTime,$endTime,12670,'Inv_Eff');
		$inver4Eff =getdeviceData($startTime,$endTime,12675,'Inv_Eff');
    ...
    $inverEff1 = combineIrradWithInverterEff($inver1Eff,10142,'Solar_Radiation');
  }
  if ($inver2Eff) {
    $inver2Eff = alignTimestamps($inver2Eff);
    $inverEff2 = combineIrradWithInverterEff($inver2Eff,10142,'Solar_Radiation');

  }
  if ($inver3Eff) {
    $inver3Eff = alignTimestamps($inver3Eff);
    $inverEff3 = combineIrradWithInverterEff($inver3Eff,10142,'Solar_Radiation'); 
  }
  if ($inver4Eff) {
    $inver4Eff = alignTimestamps($inver4Eff);
    $inverEff4 = combineIrradWithInverterEff($inver4Eff,10142,'Solar_Radiation');
                   ...
                   <!-- <input title="Reset the zoom"    style="flow: left;" id="resetZoom" onClick="resetZoom()" type="image"    src="../imgs/lupe_grey.png" disabled>-->
                    ...
                    if ($exception != 25) {
                        if ($anyArgs) {
							...
							if($phase == "LAPINV1EFF" && $park_no=="55"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,12669,Inv1_Eff,Inverter1,15,%,8;0,1,0,10142,Solar_Radiation,Irradiation,5,W/m&sup2;,\'Gold\';">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
							
							if($phase == "LAPINV2EFF" && $park_no=="55"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,12672,Inv2_Eff,Inverter2,15,%,8;0,1,0,10142,Solar_Radiation,Irradiation,5,W/m&sup2;,\'Gold\';">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
							if($phase == "LAPINV3EFF" && $park_no=="55"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,12670,Inv3_Eff,Inverter3,15,%,8;0,1,0,10142,Solar_Radiation,Irradiation,5,W/m&sup2;,\'Gold\';">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}
							if($phase == "LAPINV4EFF" && $park_no=="55"){
								echo '<a href="amplusExport.php?stamp='.$stamp.'&endstamp='.$endstamp.'&phase=' . $phase .'&args=0,1,0,12675,Inv4_Eff,Inverter4,15,%,8;0,1,0,10142,Solar_Radiation,Irradiation,5,W/m&sup2;,\'Gold\';">';
								echo '    <img title="Export Meter data as .csv file" src="../imgs/xls.png">';
								echo '</a>';
							}							
...
<?php if(($phase == 'LAPINV1EFF' && $park_no == 55) || ($phase == 'LAPINV2EFF' && $park_no == 55)|| ($phase == 'LAPINV3EFF' && $park_no == 55) || ($phase == 'LAPINV4EFF' && $park_no == 55)){?>
	$(function() {

			var datasets = {
			"Inv Efficency": {
				
				<?php if($phase == 'LAPINV1EFF'){?>
				label: "Inv1 Efficency", points: { symbol: "circle" },
				data: <?php echo $inverEff1; ?>
				<?php } else if($phase == 'LAPINV2EFF'){?>
				label: "Inv2 Efficency", points: { symbol: "circle" },
				data: <?php echo $inverEff2; ?>
				<?php } else if($phase == 'LAPINV3EFF'){?>
				label: "Inv3 Efficency", points: { symbol: "circle" },
				data: <?php echo $inverEff3; ?>
				<?php } else if($phase == 'LAPINV4EFF'){?>
	...
	<?php }elseif(($phase == 'LAPINV1ACPR' && $park_no == 55) || ($phase == 'LAPINV2ACPR' && $park_no == 55) || ($phase == 'LAPINV3ACPR' && $park_no == 55) || ($phase == 'LAPINV4ACPR' && $park_no == 55)){?>
	$(function() {

		var data = [
			<?php if($phase == 'LAPINV1ACPR'){?>
			{ data: <?php echo $invACPR1; ?>, points: { symbol: "diamond" }, label: "INV1 AC PR (%)" }
			<?php } else if($phase == 'LAPINV2ACPR'){?>
			{ data: <?php echo $invACPR2; ?>, points: { symbol: "diamond" }, label: "INV2 AC PR (%)" }
			<?php } else if($phase == 'LAPINV3ACPR'){?>
			{ data: <?php echo $invACPR3; ?>, points: { symbol: "diamond" }, label: "INV3 AC PR (%)" }
			<?php } else if($phase == 'LAPINV4ACPR'){?>
	...
	<?php }elseif(($phase == 'LAPINV1DCPR' && $park_no == 55) || ($phase == 'LAPINV2DCPR' && $park_no == 55) || ($phase == 'LAPINV3DCPR' && $park_no == 55) || ($phase == 'LAPINV4DCPR' && $park_no == 55)){?>
	$(function() {

		var data = [
			<?php if($phase == 'LAPINV1DCPR'){?>
			{ data: <?php echo $inverDCPR1; ?>, points: { symbol: "circle" }, label: "INV1 DC Voltage(V)" }
			<?php } else if($phase == 'LAPINV2DCPR'){?>
			{ data: <?php echo $inverDCPR2; ?>, points: { symbol: "circle" }, label: "INV2 DC Voltage(V)" }
			<?php } else if($phase == 'LAPINV3DCPR'){?>
			{ data: <?php echo $inverDCPR3; ?>, points: { symbol: "circle" }, label: "INV3 DC Voltage(V)" }
			<?php } else if($phase == 'LAPINV4DCPR'){?>

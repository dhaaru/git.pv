...
$iverterDCData = array();
$device =array('9323','9328','9329','9322','9324','9326','9325','9327');
...
$inverterACData1 =getdeviceData($startTime,$endTime,9323,'Pac');
$inverterDCData1 =getdeviceData1($startTime,$endTime,9323,$dcPowField);


$inverterACData2 =getdeviceData($startTime,$endTime,9328,'Pac');
$inverterDCData2 =getdeviceData1($startTime,$endTime,9328,$dcPowField);

$inverterACData3 =getdeviceData($startTime,$endTime,9329,'Pac');
$inverterDCData3 =getdeviceData1($startTime,$endTime,9329,$dcPowField);

$inverterACData4 =getdeviceData($startTime,$endTime,9322,'Pac');
$inverterDCData4 =getdeviceData1($startTime,$endTime,9322,$dcPowField);

$inverterACData5 =getdeviceData($startTime,$endTime,9324,'Pac');
$inverterDCData5 =getdeviceData1($startTime,$endTime,9324,$dcPowField);

$inverterACData6 =getdeviceData($startTime,$endTime,9326,'Pac');
$inverterDCData6 =getdeviceData1($startTime,$endTime,9326,$dcPowField);

$inverterACData7 =getdeviceData($startTime,$endTime,9325,'Pac');
$inverterDCData7 =getdeviceData1($startTime,$endTime,9325,$dcPowField);

$inverterACData8 =getdeviceData($startTime,$endTime,9327,'Pac');
$inverterDCData8 =getdeviceData1($startTime,$endTime,9327,$dcPowField);
...
foreach ($inverterDCVolt as $inverterDCVoltageData){
	...
	$stime= $dcindex1-90;
	$etime=$dcindex1+90;
	$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='9316') and (field='Solar_Radiation') and value > 600");
			...
			if($numCount1!=0)
				{	 
					...
					if($ts1>=$stime || $ts1<=$etime){
						...
						$DC_Vol_Coeff_Val= round((($dcVoltage[0]/(21*44.8))*100),2);
				...
				if($acindex1 == $dcindex1)
					{
						...
						$stime= $acindex1-90;
						$etime= $acindex1+90;
						$qry_irrad1=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='9316') and (field='Solar_Radiation') and value > 250");
						$numCount1 =  mysql_num_rows($qry_irrad1); 
						if($numCount1!=0)
						{
							$fetchdata1=mysql_fetch_array($qry_irrad1);
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts1 and (device='9316') and (field='Module_Temperature')");
							...
							$expPower=$irradation1 * 84 *(2.244) * (0.151);
							$actualPower = $acPower[0];
							$pr= round((($actualPower/$expPower)*100),2);
							
							$insq1="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld."','".$pr."'),('".$acindex1."',9316,'".$inv_irr."','".$irradation1."'),('".$acindex1."',9316,'".$ACmodule_fld1."','".$module_tempVal1."')";
							echo "<br>".$insq1."<br>";
							$iqry1= mysql_query($insq1) or die(mysql_error());
						}
						
						//////above 600
						$qry_irrad2=mysql_query("select  ROUND((value),3) as groupval,ts FROM _devicedatavalue where ts BETWEEN $stime AND $etime and (device='9316') and (field='Solar_Radiation') and value > 600");
						...
						if($numCount2!=0)
						{
							...
							$qry_module=mysql_query("select  value FROM _devicedatavalue where ts=$ts2 and (device='9316') and (field='Module_Temperature')");
							$fetchdata3=mysql_fetch_array($qry_module);
							$module_tempVal2 =$fetchdata3['value'];
							$ACmodule_fld2="AC_Module_Temp_600";
							$pr_fld2="Inv_AC_PR";
							$inv_irr2 = "Inv_irrad_600";
							$expPower=$irradation2 * 84 * (2.244) * (0.151);
							$actualPower = $acPower[0];
							$pr2= round((($actualPower/$expPower)*100),2);
							//print_r($acPower[0]);echo '/'.$expPower.'<br>';
							
							$insq2="REPLACE ".$table." (ts,device,field,value) values('".$acindex1."','".$device."','".$pr_fld2."','".$pr2."'),('".$acindex1."',9316,'".$inv_irr2."','".$irradation2."'),('".$acindex1."',9316,'".$ACmodule_fld2."','".$module_tempVal2."')";
							echo "<br>".$insq2."<br>";
...
//Energy meter system PR
  ...
  $qry_irrad4=mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts > $startTime and ts < $endTime and  (device='9316') and (field='Solar_Radiation') and value > 250");
   
   while($rlt = mysql_fetch_array($qry_irrad4)){
  	 $timestamp = $rlt ['ts'];
  	 $En_irradation1 = $rlt['groupval'];

		  $qry_enacpower = mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='9317') and (field='Activepower_Total')");
      ...
      $qry_temp =mysql_query("select ts, ROUND((value),3) as groupval FROM _devicedatavalue where ts=$timestamp and (device='9316') and (field='Module_Temperature')");
      $numCount2 =  mysql_num_rows($qry_temp); 
		...
		$SPR = round((($En_ActPowValue)/(($En_irradation1*672*(2.244)*0.151)/1000))*100,2); 
        $insq4="REPLACE ".$table."(ts,device,field,value) values('".$timestamp."',9317,'".$System_PR."','".$SPR."'),('".$timestamp."',9316,'".$Energy_irrad."','".$En_irradation1."'),('".$timestamp."', 9316 ,'".$En_Module_Temp."','".$En_ModuleValue."'),('".$timestamp."', 9317 ,'".$En_ActPowFld."','".$En_ActPowValue."')";
		//echo "<br>".$insq4."<br>";
